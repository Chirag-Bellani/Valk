...
const RepostLoad = ({navigation, route}) => {
  const GOOGLE_MAPS_API_KEY = 'AIzaSyBuUVyHOxiZyUIvBIvsZg6O_ZiedhxW0FA';

  const formateRepostDatedisplay = date => {
  ...
  const [loadingPoint, setLoadingPoint] = useState(
    repostLoadData?.from_location ?? '',
  );
  const [dropPoint, setDropPoint] = useState(repostLoadData?.to_location ?? '');
  const [quantity, setQuantity] = useState(repostLoadData?.qty ?? '');
  const [unit, setUnit] = useState(repostLoadData?.unit ?? 'Kilo Litre(s)');
  ...
  const [emptyParkName, setEmptyParkName] = useState(
    repostLoadData?.empty_park ?? '',
  );
  const [isSelectedInsurance, setSelectionInsurance] = useState(
    repostLoadData?.is_cargo_insurance === 1 ? true : false,
  );
  const [isInsuranceNeeded, setIsInsuranceNeeded] = useState(
    repostLoadData?.is_cargo_insurance ?? null,
  );
  const [cargoId, setCargoId] = useState(repostLoadData?.get_cargo.id ?? null);
  const [vehicleCategoryId, setVehicleCategoryId] = useState(
    repostLoadData?.vehicle_category_id ?? null,
  );
  const [selectedCard, setSelectedCard] = useState(
    repostLoadData?.vehicle_category_id ?? null,
  );
  const [vehicleTypeId, setVehicleTypeId] = useState(
    repostLoadData?.vehicle_type_id ?? null,
  );
  const [vehicleSizeId, setVehicleSizeId] = useState(
    repostLoadData?.vehicle_size_id ?? null,
  );
  const [labourCharge, setLabourCharge] = useState(
    repostLoadData?.labour_charges ?? 0,
  );
  const [selectedBoxLabour, setSelectedBoxLabour] = useState(
    repostLoadData?.is_labour_charges === 0 ? 2 : 1,
  );
  const [selectedBox, setSelectedBox] = useState(
    repostLoadData?.price_type === 'Fixed Price'
      ? 1
      : repostLoadData?.price_type === 'Per Tonne'
      ? 2
      : 3,
  );
  const [labourChargeBox, setLabourChargeBox] = useState(
    repostLoadData?.is_labour_charges === 0 ? 'No' : 'Yes',
  );
  const [expectedPrice, setExpectedPrice] = useState(
    repostLoadData?.expected_price ?? '',
  );
  const [isTandCSelected, setIsTandCSelected] = useState(
    repostLoadData?.is_applied_t_n_c === 1 ? true : false,
  );
  const [isTandCAgree, setIsTandCAgree] = useState(
    repostLoadData?.is_applied_t_n_c ?? null,
  );
  const [advancePerValue, setAdvancePerValue] = useState(
    repostLoadData?.advance_per ?? 50,
  );
  const [consignorId, setConsignorId] = useState(
    repostLoadData?.consignor_id ?? null,
  );
  const [consigneeId, setConsigneeId] = useState(
    repostLoadData?.consignee_id ?? null,
  );
  const [selectedUnit, setSelectedUnit] = useState(
    repostLoadData?.unit === 0 ? 2 : 1,
  );
  const [isSelected, setSelection] = useState(
    repostLoadData?.is_odc_consignment === 1 ? true : false,
  );
  const [isOdcConsignment, setIsOdcConsignment] = useState(
    repostLoadData?.is_odc_consignment === 1 ? true : false,
  );
  const [length, setLength] = useState(repostLoadData?.length ?? null);
  const [height, setHeight] = useState(repostLoadData?.height ?? null);
  const [breadth, setBreadth] = useState(repostLoadData?.breadth ?? null);
  const [fromLocationState, setFromLocationState] = useState(
    repostLoadData?.from_location_state ?? '',
  );
  const [toLocationState, setToLocationState] = useState(
    repostLoadData?.to_location_state ?? '',
  );
  ...
  useEffect(() => {
    if (repostLoadData?.pickup_time) {
      const formattedTime = moment(
        repostLoadData?.pickup_time,
        'HH:mm:ss',
      ).format('hh:mm A');
      setDisplayValue(formattedTime);
    }
  }, [repostLoadData?.pickup_date, repostLoadData?.pickup_time]);
              ...
              <View style={mainStyles.bottomMargin}>
                <View style={mainStyles.checkboxContainer}>
                  <CheckBox
                    ...
                    tintColors={{true: '#203afa'}}
                  />
                  <Text style={mainStyles.label}>Cargo Insurance Require</Text>
                </View>
              </View>
              ...
              <View style={mainStyles.bottomMargin}>
                <View style={mainStyles.checkboxContainer}>
                  <CheckBox
                    ...
                    tintColors={{true: '#203afa'}}
                  />
                  <Text style={mainStyles.label}>Add ODC Consignment</Text>
                </View>
              </View>

              {isOdcConsignment === true && (
                <>
                  <View
                    style={{
                      flexDirection: 'row',
            ...
            <TouchableWithoutFeedback onPress={() => {}}>
              <KeyboardAvoidingView
                ...
                <TextInput
                  ...
                  value={addCargoName}
                  multiline={true}
                  onChangeText={text => setAddCargoName(text)}
                />
